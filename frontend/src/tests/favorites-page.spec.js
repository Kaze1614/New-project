    ...
    children: [
      {
        ...
        title: 'Limits',
        children: [
          { id: 5, title: 'Limit Basics', children: [] },
          { id: 6, title: 'Infinitesimals', children: [] }
        ]
      }
    ]
  }
...
describe('FavoritesPage', () => {
  ...
  it('renders favorite cards with filters and review action', async () => {
    const requestParams = []
          ...
          data: [
            {
              ...
              difficulty: 'HARD',
              title: 'Limit card',
              content: 'sinx/x when x -> 0 equals ?',
              questionType: 'SINGLE',
    ...
    const wrapper = mount(FavoritesPage, { global: { plugins: [createPinia()] } })
    await flushPromises()

    expect(wrapper.find('.favorites-filter-bar').exists()).toBe(true)
    expect(wrapper.find('.favorites-grid').exists()).toBe(true)
    expect(wrapper.text()).toContain('sinx/x when x -> 0 equals ?')
    expect(wrapper.text()).toContain('Limit Basics')
  ...
  it('supports multi-choice and fill-answer cards', async () => {
          ...
          data: [
            {
              ...
              difficulty: 'MEDIUM',
              title: 'Set card',
              content: 'Elements in set A are:',
              questionType: 'MULTI',
              options: ['A. 1', 'B. 2', 'C. 3', 'D. 4'],
              correctAnswer: 'A,C',
              explanation: 'Correct choices are A and C.',
              sourceLabel: 'Textbook Example',
              createdAt: '2026-04-26T10:00:00'
            },
            {
              ...
              difficulty: 'EASY',
              title: 'Fill card',
              content: 'If f(x)=2x+1, then f(3)=?',
              questionType: 'FILL',
              options: [],
              correctAnswer: '7',
              explanation: 'Substitute directly and get 7.',
              sourceLabel: 'Textbook Example',
    ...
    expect(fillCard.text()).toContain('Substitute directly and get 7.')
  })
